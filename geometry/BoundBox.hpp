...
struct BoundBox { 
	...
	const Point3D offset(const Point3D&) const;
	Point3D diagonal();
	const int MaximumExtent();
	float surfaceArea();
    inline bool intersects(BoundBox&) const;
...
const Point3D BoundBox::offset(const Point3D& p) const {
...
bool BoundBox::intersects(BoundBox& box) const {
    const BoundBox &bounds = *this;
    Point3D centroid = Point3D((box.pMin.x + box.pMax.x) * 0.5f, (box.pMin.y + box.pMax.y) * 0.5f, (box.pMin.z + box.pMax.z) * 0.5f);
    return bounds.pMin < centroid && bounds.pMax > centroid;
}
bool BoundBox::operator==(const BoundBox &b) const {
